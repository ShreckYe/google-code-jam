...
fun testCase(ti: Int) {
    ...
    val xx = List(nn) { readLine()!!.splitToSequence(' ').map { it.toLong() }.toList() }

    val n = nn + 1
    val minMaxXs = listOf(longArrayOf(0L, 0L)) + xx.map { longArrayOf(it.minOrNull()!!, it.maxOrNull()!!) }
    ...
    val y = dp[n - 1].minOf { it.minOrNull()!! } + minMaxXs.map { (it[1] - it[0]) }.sum()
    println("Case #${ti + 1}: $y")
}
